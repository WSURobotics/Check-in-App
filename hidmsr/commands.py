...
# TODO: Make this part of MSRDevice and integrate with _send_command_wait.
...
class MSRDevice():
    ...
    def __init__(self, vendor_id=0x0801, product_id=0x0003):
        ...
        # We end up using a raw data handler here since it seems that pywinusb doesn't
        # send back the correct data whenever we need it. This may be something that we
        # can fix with a bug report. I'll need to investigate this when I can.
        data_handler = functools.partial(_data_handler, self)
        device.set_raw_data_handler(data_handler)
        _l.debug("Created MSR device.")

    def _send_command_wait(self, command, wait_time=0.15):
    ...
    def msr_model(self):
        ...
        return self._send_command_wait([0xC5, 0x1B, 0x74])
    ...
    def unknown1(self):
        ...
        return self._send_command_wait([0xC5, 0x1B, 0xA9])

    def unknown2(self):
        """ Unknown device function. Included for the sake of completeness.
        This method is not known to return anything.
        """
        _l.warning("This is an unknown function. Use at your own risk!")
        self._send_command_nowait([0xC5, 0x1B, 0xA1])

    def unknown3(self):
        """ Unknown device function. Included for the sake of completeness. """
        _l.warning("This is an unknown function. Use at your own risk!")
        return self._send_command_wait([0xC5, 0x1B, 0xA2])

    def unknown4(self):
        """ Unknown device function. Included for the sake of completeness. """
        _l.warning("This is an unknown function. Use at your own risk!")
        return self._send_command_wait([0xC5, 0x1B, 0xA3])
    ...
    def unknown5(self):
        ...
        """
        _l.warning("This is an unknown function. Use at your own risk!")
        self._send_command_nowait([0xC5, 0x1B, 0xA4])

    def unknown6(self):
        """ Unknown device function. Included for the sake of completeness. """
        _l.warning("This is an unknown function. Use at your own risk!")
        return self._send_command_wait([0xC5, 0x1B, 0xAE])
